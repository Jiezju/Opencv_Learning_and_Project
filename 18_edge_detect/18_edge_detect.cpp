...
int main(int argc, char **argv) {
    ...
    namedWindow("【效果图】Laplacian 边缘检测", WINDOW_AUTOSIZE);

    Mat dst;
    GaussianBlur(g_srcGrayImage, dst, Size(3, 3), 0, 0, BORDER_DEFAULT);

    Laplacian(dst, dst, CV_16S, 3, 1, 0, BORDER_DEFAULT);
    convertScaleAbs(dst, dst);
    imshow("【效果图】Laplacian 检测效果", dst);
